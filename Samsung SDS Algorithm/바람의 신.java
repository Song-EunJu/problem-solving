/**
 ...
 -> 근데 이 문제는 한번 옮겼을 때의 해가 문제에 대한 최적해가 될 수 없지 않을까 싶은데


 -> 일단 값들을 모으는게 우선일 것 같은데,,
 -> 완탐 (브루트 포스) 는 너무 멍청한 방법인 거 같고 DP 인가

 -> 근데 이거 같은 조합이어도 순서에 따라 다른 결과가 나오나
 -> 테스트 해보니까 같은 조합이면 최댓값은 동일하게 계산이 됨
 -> 중복 조합 (중복을 허락해서 뽑을 수 있는 경우의 수)
 -> 4H3 = 6C3 = 20가지
 -> 아니 근데 같은 조합이어도 순서에 따라 다른 경우가 나오네... ㅎㅎ
